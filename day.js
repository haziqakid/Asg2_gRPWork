...
class fetchForecastApi {
    ...
    getLocation(query, callback) { // callback
        $.getJSON(this.searchApiUrl, {query})
        .done(data => callback(data))
        .fail(() => callback(null)) //data will be passed to fetchWeather(data) ||  if fails, pass to callback
    }
...
class displayForecast {
    ...
    showTodaysForecastDetails({name, value, unit}) {
        $(`#forecast-details`).append(`
            <div class="d-flex justify-content-between">
                <span class="font-weight-bolder">${name}</span><span>${value} ${unit}</span>
                </div>
                `);
    }
    ...
    showUpcomingDaysForecast({dayImgUrl, weekDay, maxTemp}) {
        ...
        <li class="forecastBox__week-day d-flex flex-column justify-content-center align-items-center p-2
        weather-day">
...
class dataMiddleware {                  // right hand side of taskbar
    constructor() {
    ...
    prepareDataForDom(data) {
       const {predictability, humidity, wind_speed, air_pressure, max_temp, min_temp, applicable_date, the_temp,
    weather_state_abbr, weather_state_name } = data.consolidated_weather[0] //first element of array
    ...
    const todaysForecastGeneral = this.gatherTodaysForecastGeneral({
        ...
        this.prepareTodaysForecastDetails(todaysForecastDetails); // call method
    ...
    prepareUpcomingDaysForecast(forecast) {
        $.each(forecast, (index, value) => {
            ...
            const weekDay = moment(value.applicable_date).format('dddd').substring(0,3); //moment.js
...
class requestController {
    ...
    fetchWeather(query) {
        this.fetchForecastApi.getLocation(query, location => {
            ...
            this.fetchForecastApi.getWeatherData(location[0].woeid, data => { //where on earth id, check if data we need is there
                if (!data) {
                    this.coreDomElements.showError('Could not proceed with your request, please try again later !');  
                    return;                  
                }

                this.dataMiddleware.prepareDataForDom(data) // if response is successful
            })
        });
    }
    ...
    registerEventListener(){ 
        this.coreDomElements.searchForm.on('submit', (e) => {
            e.preventDefault(); // prevent reloading wp
            this.onSubmit();
        });
    }
}
